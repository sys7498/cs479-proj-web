...
class Viewer {
  ...
  init() {
    ...
    this.camera.position.copy(
      this.initialCameraPos.add(new THREE.Vector3(0, 0, 0))
    );
    //this.camera.lookAt(this.initialCameraLookAt);
    //this.camera.rotateZ(Math.PI); // Z축을 기준으로 180도 회전
...
function init() {
  function load() {
    document.addEventListener("DOMContentLoaded", () => {
      ...
      const summaryContents = document.getElementById("summaryContents");
      ...
      toggleBtn.addEventListener("click", () => {
        ...
        if (isVisible) {
          ...
          toggleBtn.textContent = "click to hide contents";
        } else {
          summaryContents.classList.add("hidden");
          toggleBtn.textContent = "click to show contents";
        }
      });
    });
    let viewers = [];
